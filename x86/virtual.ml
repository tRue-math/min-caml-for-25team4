...
let rec g env ({v=e;pos}:Closure.t) = (* 式の仮想マシンコード生成 (caml2html: virtual_g) *)
  ...
  | Closure.Float(d) ->
      let l =
        ...
        with Not_found ->
          ...
          l in
      let x = Id.genid "l" in
      set_pos (Let((x, Type.Int), SetL(l), set_pos (Ans(LdDF(x, C(0), 1)))))
  | Closure.Neg(x) -> set_pos (Ans(Neg(x)))
  | Closure.Add(x, y) -> set_pos (Ans(Add(x, V(y))))
  | Closure.Sub(x, y) -> set_pos (Ans(Sub(x, V(y))))
  | Closure.FNeg(x) -> set_pos (Ans(FNegD(x)))
  | Closure.FAdd(x, y) -> set_pos (Ans(FAddD(x, y)))
  | Closure.FSub(x, y) -> set_pos (Ans(FSubD(x, y)))
  | Closure.FMul(x, y) -> set_pos (Ans(FMulD(x, y)))
  | Closure.FDiv(x, y) -> set_pos (Ans(FDivD(x, y)))
  ...
  | Closure.Var(x) ->
      ...
      | Type.Float -> set_pos (Ans(FMovD(x)))
      | _ -> set_pos (Ans(Mov(x))))
  ...
  | Closure.LetTuple(xts, y, e2) ->
      ...
      let (offset, load) =
        expand
          ...
          (fun x t offset load ->
            if not (S.mem x s) then load else (* [XX] a little ad hoc optimization *)
            set_pos (Let((x, t), Ld(y, C(offset), 1), load))) in
      load
  | Closure.Get(x, y) -> (* 配列の読み出し (caml2html: virtual_get) *)
      (match M.find x env with
      | Type.Array(Type.Unit) -> set_pos (Ans(Nop))
      | Type.Array(Type.Float) -> set_pos (Ans(LdDF(x, V(y), 8)))
      | Type.Array(_) -> set_pos (Ans(Ld(x, V(y), 4)))
      | _ -> assert false)
  | Closure.Put(x, y, z) ->
      (match M.find x env with
      | Type.Array(Type.Unit) -> set_pos (Ans(Nop))
      | Type.Array(Type.Float) -> set_pos (Ans(StDF(z, x, V(y), 8)))
      | Type.Array(_) -> set_pos (Ans(St(z, x, V(y), 4)))
      | _ -> assert false)
...
let h { Closure.name = (Id.L(x), t); Closure.args = yts; Closure.formal_fv = zts; Closure.body = {v=_;pos} as e } =
  ...
  let (offset, load) =
    expand
      ...
      (fun z t offset load -> {v=(Let((z, t), Ld(x, C(offset), 1), load));pos}) in
  match t with
  | Type.Fun(_, t2) ->
